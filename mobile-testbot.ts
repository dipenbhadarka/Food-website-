...
const localNoReset = (process.env.LOCAL_NO_RESET || 'false').toLowerCase() === 'true';
...
export function getTestBotServices(): ServiceOption[] {
    ...
    return [['browserstack', {
        ...
        app: process.env.BROWSERSTACK_APP || process.env.LOCAL_APP_PATH,
...
export function getTestBotCapabilities(): RequestedStandaloneCapabilities[] {
    ...
    if (context.platform === TestBotPlatform.Android) {
        ...
        if (isLocal) {
            ...
            console.log(` Local Android reset mode: ${localNoReset ? 'NO_RESET' : 'CLEAN_START'}`)

            const localAppPath = process.env.LOCAL_APP_PATH;

            if (localAppPath) console.log(` Installing APK: ${localAppPath}`);
            ...
            return [{
                ...
                ...(localAppPath
                    ...
                    ? {
                        ...
                        'appium:app': localAppPath,

                        // fullReset uninstalls+reinstalls BEFORE UiAutomator2 starts

                        // — clean app state without killing the instrumentation.

                        'appium:fullReset': false,

                        'appium:noReset': true,

                        // Prevent the app's background network calls from blocking findElements.

                        'appium:settings': { wakeLockForSession: true, waitForIdleTimeout: 500 },
    ...
    // ── iOS ───────────────────────────────────
        ...
        if (isLocal) {
            ...
            console.log(` Local iOS reset mode: ${localNoReset ? 'NO_RESET' : 'CLEAN_START'}`)
